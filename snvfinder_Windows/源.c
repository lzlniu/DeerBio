...
date:2020/05/09
platform:Windows
utility:extract snvs position from multi slim fasta file(already complete the global alignment) and forming multiple circos configuration ".txt".
...
int main() {
    ...
    out = fopen("1.txt", "w");
    if (out == NULL) { printf("cann't open 1.txt!\n"); return 0; }
    ...
    while (!feof(in)) {
        ...
        if (nucl == '.' && len > 1) {
            fprintf(out, "chr1\t%d\t%d\tfill_color=red\n", i - len, j);
            len = 0;
        }
        else if (nucl == '.' && len == 1) {
            fprintf(out, "chr1\t%d\t%d\tfill_color=red\n", j, j);
